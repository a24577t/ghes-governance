...
**Execution Status**:
The closed execution-level completeness result recorded in Evidence: Complete, CompleteWithGaps, or Failed, with accounting of discovered, evaluated, and Unknown counts. Each Unknown is recorded with its Unknown Classification, and the status derives from that classification: CompleteWithGaps when any recorded Unknown is IncompleteObservation, otherwise Complete (a Failed execution aborts before completing authoritative evaluation).

**Unknown Classification**:
The closed two-member classification recorded for every Unknown in completeness accounting, distinguishing why it arose: IncompleteObservation — the Unknown follows from a CannotDetermine provider result, so observation of the declared Evaluation Scope was incomplete (scope-undetermined applicability and authority-undeterminable outcomes); or GovernanceResult — the Unknown is a determined governance verdict over fully observed inputs (authority conflict; unregistered evaluation strategy). Execution Status derives from it. It is the minimum classification needed to derive Execution Status by reason, never a general reason taxonomy (ADR-0015). The classification and the accounting count attach to the causal evaluation record that produces one or more Unknown dimensions, counted once even when both the Policy Outcome and the Coverage State inherit Unknown from it: the (repository, policy) pair authority-selection result for the terminal authority cases, or the requirement-evaluation record for a predicate CannotDetermine.
