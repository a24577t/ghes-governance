...
**Unit.** The classification and the accounting count attach to the *causal evaluation record* that produces one or more `Unknown` dimensions — the (repository, policy) pair authority-selection result for the terminal authority cases, and the requirement-evaluation record for a predicate `CannotDetermine` — counted **once** even when both Policy Outcome and Coverage State inherit `Unknown` from it.
