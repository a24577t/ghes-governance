...
Excluded requirements are compliance-neutral in aggregation and coverage-significant: a policy may report `Compliance: Compliant / Coverage: GovernanceExclusion`, and that is never presented as equivalent to full compliance with full coverage. ADR-0007's closed coverage set accordingly includes `GovernanceExclusion`.

## Lifecycle

Every relief artifact carries an explicit lifecycle: `Draft → Approved → Active → Expiring → Expired`, with `Superseded` when replaced by a newer version. Draft and Approved are Git-process states (pre-merge; merged but not yet effective); the engine derives Active, Expiring, Expired, and Superseded at the fixed evaluation timestamp (ADR-0005).
...
## Scope reduction is not relief

Reducing an authoritative binding's scope expression is not a sanctioned risk-acceptance mechanism and is never treated as equivalent to a Governance Exclusion — it carries no expiry and no coverage signal. Scope-reducing changes to authoritative bindings are governance-significant: a future control must produce a scope-diff report identifying repositories removed from governance. Historical evidence and prior governance status remain intact when a repository leaves scope. (Control requirement recorded now; not implemented in the first slice.)
...
- Unknown results may never be overridden by governance artifacts.
